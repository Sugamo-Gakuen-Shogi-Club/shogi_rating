import {
  ...
  SyncStatus, SyncMeta, AutoBackupEntry, UndoSnapshot,
  RankApplication, OfficialRank
} from './types';
...
const UNDO_KEY          = 'club_rivals_undo';
const RANK_APP_KEY      = 'club_rivals_rank_apps';
const AUTOBACKUP_PREFIX = 'club_rivals_backup_';
const MAINT_BACKUP_KEY  = 'club_rivals_maint_backup';
const LOGS_MAX          = 200;
const UNDO_MAX          = 15;
...
export const ACHIEVEMENTS_DATA: AchievementDef[] = [
  ...
  { id: 'RATE_200',        name: '脱・初心者',        description: 'レート200到達',             conditionType: 'RATE',     threshold: 200 },
  { id: 'RATE_500',        name: '熟練者',           description: 'レート500到達',             conditionType: 'RATE',     threshold: 500 },
  { id: 'RATE_800',        name: 'マスター',         description: 'レート800到達',             conditionType: 'RATE',     threshold: 800 },
  { id: 'RATE_1000',       name: 'レジェンド',        description: 'レート1000到達',            conditionType: 'RATE',     threshold: 1000 },
...
export const ICONS_DATA: IconDef[] = [
  ...
  { id: 'SHOGI_KA',        char: '角行', name: '角行',      conditionDescription: 'レート200到達',   type: 'RATE',     threshold: 200,  category: 'SHOGI' },
  { id: 'SHOGI_UMA',       char: '龍馬', name: '龍馬',      conditionDescription: 'レート400到達',   type: 'RATE',     threshold: 400,  category: 'SHOGI' },
  { id: 'SHOGI_HI',        char: '飛車', name: '飛車',      conditionDescription: 'レート300到達',   type: 'RATE',     threshold: 300,  category: 'SHOGI' },
  { id: 'SHOGI_RYU',       char: '龍王', name: '龍王',      conditionDescription: 'レート600到達',   type: 'RATE',     threshold: 600,  category: 'SHOGI' },
  { id: 'SHOGI_OU',        char: '王将', name: '王将',      conditionDescription: 'レート800到達',   type: 'RATE',     threshold: 800,  category: 'SHOGI' },
  { id: 'SHOGI_GYOKU',     char: '玉将', name: '玉将',      conditionDescription: 'レート1000到達',  type: 'RATE',     threshold: 1000, category: 'SHOGI' },
  ...
  { id: 'CHESS_BISHOP',    char: '♝',   name: 'ビショップ', conditionDescription: 'レート500到達',  type: 'RATE',     threshold: 500,  category: 'CHESS' },
  { id: 'CHESS_ROOK',      char: '♜',   name: 'ルーク',    conditionDescription: 'レート700到達',   type: 'RATE',     threshold: 700,  category: 'CHESS' },
  { id: 'CHESS_QUEEN',     char: '♛',   name: 'クイーン',  conditionDescription: 'レート900到達',   type: 'RATE',     threshold: 900,  category: 'CHESS' },
  { id: 'CHESS_KING',      char: '♚',   name: 'キング',    conditionDescription: 'レート1000到達',  type: 'RATE',     threshold: 1000, category: 'CHESS' },
];
...
export const getMatches = (): MatchRecord[] => {
  try { const m = localStorage.getItem(MATCHES_KEY); return m ? JSON.parse(m) : []; } catch { return []; }
};

export const getLogs = (): ActivityLog[] => {
  try { const l = localStorage.getItem(LOGS_KEY); return l ? JSON.parse(l) : []; } catch { return []; }
};
...
// UNDO SYSTEM
// ============================================================
export const getUndoSnapshots = (): UndoSnapshot[] => {
  try {
...
const saveUndoSnapshots = (snaps: UndoSnapshot[]) => {
  localStorage.setItem(UNDO_KEY, JSON.stringify(snaps.slice(0, UNDO_MAX)));
};

/** Call this BEFORE any destructive operation */
export const pushUndoSnapshot = (label: string): void => {
  ...
  const snaps = getUndoSnapshots();
  snaps.unshift(snap);
  saveUndoSnapshots(snaps);
};

/** Restore state from snapshot (does NOT push another undo level) */
export const applyUndoSnapshot = (snapId: string): boolean => {
  const snaps = getUndoSnapshots();
  const snap = snaps.find(s => s.id === snapId);
  if (!snap) return false;
  localStorage.setItem(USERS_KEY,   JSON.stringify(snap.users));
  localStorage.setItem(MATCHES_KEY, JSON.stringify(snap.matches));
  localStorage.setItem(LOGS_KEY,    JSON.stringify(snap.logs));
  // Remove this snapshot and all newer ones
  const idx = snaps.findIndex(s => s.id === snapId);
  saveUndoSnapshots(snaps.slice(idx + 1));
  syncWithServer();
  return true;
};
...
// AUTO BACKUP
// ============================================================
export const saveAutoBackup = (label?: string): void => {
  try {
    const today = getLocalDateString();
    const key = label ? `${AUTOBACKUP_PREFIX}${label}` : `${AUTOBACKUP_PREFIX}${today}`;
    ...
    if (!label) {
      ...
      Object.keys(localStorage)
        .filter(k => k.startsWith(AUTOBACKUP_PREFIX) && !k.includes('maint') && k.replace(AUTOBACKUP_PREFIX, '') < cutoffStr)
    ...
    .map(key => {
      try {
        ...
        return {
          ...
          label: key.includes('maint') ? '🔧 メンテナンス前' : undefined,
        } as AutoBackupEntry;
      } catch { return null; }
...
// MAINTENANCE MODE
// ============================================================
export const enableMaintenanceMode = async (): Promise<boolean> => {
  // 1. Snapshot current data locally
  const backupKey = `${AUTOBACKUP_PREFIX}maint_${Date.now()}`;
  saveAutoBackup(`maint_${Date.now()}`);

  // 2. Also push pre-maintenance snapshot to Firebase with special key
  try {
    const data: BackupData = {
      users: getRawUsers(), matches: getMatches(),
      settings: getSettings(), logs: getLogs(),
      timestamp: new Date().toISOString(), rankApplications: getRankApplications(),
    };
    await fetch(
      'https://club-rivals-test1-default-rtdb.asia-southeast1.firebasedatabase.app/rivals_maint_backup.json',
      { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }
    );
  } catch (_) {}

  const s = getSettings();
  saveSettings({ ...s, maintenanceMode: true, maintenanceBackupKey: backupKey });
  return true;
};

export const disableMaintenanceMode = async (restorePreMaintenance: boolean): Promise<void> => {
  const s = getSettings();
  if (restorePreMaintenance && s.maintenanceBackupKey) {
    restoreFromAutoBackup(s.maintenanceBackupKey);
    // Push restored data to Firebase
    await pushToCloud();
  }
  // Clean up maintenance backup from Firebase
  try {
    await fetch(
      'https://club-rivals-test1-default-rtdb.asia-southeast1.firebasedatabase.app/rivals_maint_backup.json',
      { method: 'DELETE' }
  ...
  // Remove local maint backup
  if (s.maintenanceBackupKey) localStorage.removeItem(s.maintenanceBackupKey);
  saveSettings({ ...s, maintenanceMode: false, maintenanceBackupKey: null });
};
...
export const pushToCloud = async (): Promise<boolean> => {
  try {
    ...
    const data: BackupData = {
      users: getRawUsers(), matches: getMatches(),
      settings: getSettings(), logs: getLogs(),
      timestamp, rankApplications: getRankApplications(),
    };
...
export const syncWithServer = (): void => {
  ...
  const cur = getSyncStatus();
  updateSyncMeta({ status: 'PENDING', localTimestamp: now, pendingChanges: (cur.pendingChanges || 0) + 1 });
  saveAutoBackup();
  ...
  _syncTimer = window.setTimeout(async () => {
    ...
    if (!ok) window.setTimeout(() => pushToCloud(), 10000);
  }, 3000);
};
...
export const loadFromCloud = async (): Promise<LoadResult> => {
  try {
    ...
    if (!data || !Array.isArray(data.users) || data.users.length === 0) {
      updateSyncMeta({ status: 'NEVER' }); return 'EMPTY';
    }
    ...
    if (localTime && cloudTime && localTime > cloudTime) {
      await pushToCloud(); return 'LOCAL_NEWER';
    }
    ...
    if (data.rankApplications) localStorage.setItem(RANK_APP_KEY, JSON.stringify(data.rankApplications));
    const syncedAt = new Date().toISOString();
    updateSyncMeta({ status: 'SYNCED', lastSync: syncedAt, localTimestamp: cloudTime, pendingChanges: 0 });
    return 'CLOUD_LOADED';
  } catch (e: any) {
    updateSyncMeta({ status: 'ERROR', lastError: e?.message }); return 'FAILED';
  }
};
...
const normalizeUser = (user: any): User => ({
  ...
  seasonStartRate:  user.seasonStartRate  ?? user.rate ?? 0,
  ...
  rateHistory:      Array.isArray(user.rateHistory) ? user.rateHistory
    : [{ date: new Date().toISOString(), rate: user.rate || 0 }],
  officialRank:     user.officialRank     ?? undefined,
});
...
const appendLogs = (newLogs: ActivityLog[]): void => {
  ...
  localStorage.setItem(LOGS_KEY, JSON.stringify([...newLogs, ...existing].slice(0, LOGS_MAX)));
};
...
const checkAchievementsAndIcons = (
  ...
  ACHIEVEMENTS_DATA.forEach(a => {
    ...
    if (met) { if (!user.achievements) user.achievements = []; user.achievements.push(a.id); newAchievements.push(a); }
  });
  ...
  ICONS_DATA.forEach(icon => {
    ...
    if (met) { if (!user.unlockedIcons) user.unlockedIcons = []; user.unlockedIcons.push(icon.id); newIcons.push(icon); }
  });
...
export const recordAttendance = (userId: string): AttendanceResult => {
  ...
  saveUsers(allUsers);
  appendLogs([{ id: randomId(), userId, type: ActivityType.ATTENDANCE, points: pts, description: '出席', date: new Date().toISOString() }]);
  return { success: true, newAchievements: res.newAchievements, newIcons: res.newIcons, message: `出席しました！ (+${pts}pt)` };
};
...
/** Returns delta (can be negative for loser) */
...
const spamPenaltyFor = (userId: string): number => {
  ...
  const recent = getMatches().filter(m =>
    (m.player1Id === userId || m.player2Id === userId) && new Date(m.date).getTime() > cutoff
  );
...
export const processMatch = (
  ...
  // Elo delta — positive for winner, negative for loser
  ...
  // Undo snapshot BEFORE modifying
  const p1Name = p1.name, p2Name = p2.name;
  const resultLabel = p1IsWinner ? `${p1Name}勝利` : p2IsWinner ? `${p2Name}勝利` : '引き分け';
  pushUndoSnapshot(`対局: ${p1Name} vs ${p2Name}（${resultLabel}）`);
  ...
  // Streaks
  if (p1IsWinner) {
    p1.currentStreak++; if (p1.currentStreak > p1.maxStreak) p1.maxStreak = p1.currentStreak; p2.currentStreak = 0;
  } else if (p2IsWinner) {
    p2.currentStreak++; if (p2.currentStreak > p2.maxStreak) p2.maxStreak = p2.currentStreak; p1.currentStreak = 0;
  } else {
    p1.currentStreak = 0; p2.currentStreak = 0;
  }

  // Rate — losers CAN go negative but floor at 0
  ...
  // Points (always positive, even for losers)
  p1.totalPoints    += p1Detail.total; p1.monthlyPoints = (p1.monthlyPoints || 0) + p1Detail.total;
  p1.pointsMatch     = (p1.pointsMatch || 0) + p1Detail.total;
  p2.totalPoints    += p2Detail.total; p2.monthlyPoints = (p2.monthlyPoints || 0) + p2Detail.total;
  p2.pointsMatch     = (p2.pointsMatch || 0) + p2Detail.total;
  if (eventActive) {
    p1.eventPoints = (p1.eventPoints || 0) + p1Detail.total;
    p2.eventPoints = (p2.eventPoints || 0) + p2Detail.total;
  }
  ...
  const matchRecord: MatchRecord = {
    ...
    player1Id: p1Id, player2Id: p2Id, result,
    p1RateChange, p2RateChange,
    p1PointsEarned: p1Detail.total, p2PointsEarned: p2Detail.total, isDuel,
  };
  ...
  appendLogs([
    { id: randomId(), userId: p1Id, type: p1IsWinner ? ActivityType.MATCH_WIN : isDraw2 ? ActivityType.MATCH_DRAW : ActivityType.MATCH_LOSS, points: p1Detail.total, description: `対局 vs ${p2.name}`, date: now },
    { id: randomId(), userId: p2Id, type: p2IsWinner ? ActivityType.MATCH_WIN : isDraw2 ? ActivityType.MATCH_DRAW : ActivityType.MATCH_LOSS, points: p2Detail.total, description: `対局 vs ${p1.name}`, date: now },
  ]);
...
export const getRivalryStats = (userId: string): { bestCustomer: RivalData | null; nemeses: RivalData | null } => {
  ...
  getMatches().forEach(m => {
    ...
    if (m.result === 'DRAW') { map[oppId].draws++; }
    else if ((m.player1Id === userId && m.result === 'PLAYER1_WIN') || (m.player2Id === userId && m.result === 'PLAYER2_WIN')) { map[oppId].wins++; }
    else { map[oppId].losses++; }
  });
  const users = getRawUsers();
  const rivals: RivalData[] = Object.entries(map).map(([oppId, s]) => ({
    opponentId: oppId, opponentName: users.find(u => u.id === oppId)?.name || 'Unknown',
    wins: s.wins, losses: s.losses, draws: s.draws, total: s.wins + s.losses + s.draws,
  ...
  return {
    bestCustomer: byWinDiff[0]?.wins  > byWinDiff[0]?.losses  ? byWinDiff[0]  : null,
    nemeses:      byLossDiff[0]?.losses > byLossDiff[0]?.wins ? byLossDiff[0] : null,
  };
};
...
export const snapshotSeasonBaseline = (): void => {
  const all = getRawUsers();
  all.forEach(u => { u.seasonStartRate = u.rate; u.seasonStartPoints = u.totalPoints; });
  saveUsers(all);
};
...
export const awardSystemTitles = (): void => {
  ...
  const byGrowth = [...active].filter(u => u.id !== master?.id)
    .sort((a, b) =>
  ...
  const byActivity = [...active].filter(u => !taken1.has(u.id))
    .sort((a, b) => ((b.wins + b.losses + b.draws) + b.activityDays) - ((a.wins + a.losses + a.draws) + a.activityDays));
  ...
  getMatches().forEach(m => {
    if (m.result === 'PLAYER1_WIN' && (rateMap[m.player2Id] ?? 0) > (rateMap[m.player1Id] ?? 0))
      upsetCount[m.player1Id] = (upsetCount[m.player1Id] || 0) + 1;
    else if (m.result === 'PLAYER2_WIN' && (rateMap[m.player1Id] ?? 0) > (rateMap[m.player2Id] ?? 0))
      upsetCount[m.player2Id] = (upsetCount[m.player2Id] || 0) + 1;
  });
  const byUpsets = [...active].filter(u => !taken2.has(u.id)).sort((a, b) => (upsetCount[b.id] || 0) - (upsetCount[a.id] || 0));
...
// ============================================================
// FACTION / GENERALS  ← Fixed: achievement check on general assignment
// ============================================================
export const getFactionBalanceSimulation = (users: User[]) => {
  const scored = users.filter(u => u.isActive !== false)
    .map(u => ({ ...u, _score: u.rate * 0.3 + (u.activityDays || 0) * 300 }))
    .sort((a, b) => b._score - a._score);
  const red: User[] = [], white: User[] = [];
  let rScore = 0, wScore = 0;
  scored.forEach(u => { if (rScore <= wScore) { red.push(u); rScore += u._score; } else { white.push(u); wScore += u._score; } });
...
export const assignGenerals = (redId: string, whiteId: string): void => {
  ...
  const r = all.find(u => u.id === redId);   if (r) { r.isGeneral = true; checkAchievementsAndIcons(r); }
  const w = all.find(u => u.id === whiteId); if (w) { w.isGeneral = true; checkAchievementsAndIcons(w); }
  saveUsers(all);
};

export const resetEventPoints = (): void => {
  const all = getRawUsers(); all.forEach(u => { u.eventPoints = 0; }); saveUsers(all);
};
...
export const deactivateUser = (id: string): void => {
  const all = getRawUsers(); const u = all.find(x => x.id === id);
...
export const getInactiveUsers = (): User[] => getRawUsers().filter(u => u.isActive === false);

// ============================================================
// MANUAL ADJUSTMENTS
// ============================================================
export const manualPointAdjustment = (uid: string, pts: number, reason: string): void => {
  const all = getRawUsers(); const u = all.find(x => x.id === uid); if (!u) return;
  u.totalPoints += pts; u.pointsSpecial = (u.pointsSpecial || 0) + pts; u.monthlyPoints += pts;
  checkAchievementsAndIcons(u); saveUsers(all);
  appendLogs([{ id: randomId(), userId: uid, type: ActivityType.BONUS, points: pts, description: reason || '管理者付与', date: new Date().toISOString() }]);
};

export const manualRateAdjustment = (uid: string, delta: number, _reason: string): void => {
  const all = getRawUsers(); const u = all.find(x => x.id === uid); if (!u) return;
  u.rate = Math.max(0, u.rate + delta);
  u.rateHistory = [...(u.rateHistory || []), { date: new Date().toISOString(), rate: u.rate }];
  checkAchievementsAndIcons(u); saveUsers(all);
};

export const resetMonthly = (): void => {
  const all = getRawUsers(); all.forEach(u => { u.monthlyPoints = 0; }); saveUsers(all);
};
...
export const updateUserReading = (id: string, reading: string): void => {
  const all = getRawUsers(); const u = all.find(x => x.id === id); if (u) { u.reading = reading; saveUsers(all); }
};
export const updateUserTitle = (id: string, t: string | null): void => {
  const all = getRawUsers(); const u = all.find(x => x.id === id); if (u) { u.activeTitle = t; saveUsers(all); }
};
export const updateUserIcon = (id: string, iconId: string): void => {
  const all = getRawUsers(); const u = all.find(x => x.id === id); if (u) { u.activeIconId = iconId; saveUsers(all); }
};
...
const newUserBase = (name: string, reading?: string, isNewMember = false): User => ({
  id: randomId(), name, reading, isActive: true, isNewMember,
  rate: 0, seasonStartRate: 0, seasonStartPoints: 0,
  faction: 'WHITE', isGeneral: false, systemTitle: null,
  totalPoints: 0, pointsMatch: 0, pointsAttendance: 0, pointsSpecial: 0,
  monthlyPoints: 0, eventPoints: 0, currentStreak: 0, maxStreak: 0,
  wins: 0, losses: 0, draws: 0, lastAttendance: null, activityDays: 0,
  rateHistory: [{ date: new Date().toISOString(), rate: 0 }],
  achievements: [], activeTitle: null,
  avatarColor: `bg-${['red','blue','green','yellow','purple','pink'][Math.floor(Math.random()*6)]}-500`,
  activeIconId: 'DEFAULT_INITIAL', unlockedIcons: [...DEFAULT_UNLOCKED_ICONS],
});

export const bulkAddUsers = (stubs: Partial<User>[]): void => {
  const all = getRawUsers();
  saveUsers([...all, ...stubs.map(s => newUserBase(s.name || '名称未設定', s.reading, !!s.isNewMember))]);
};
...
export const getRankApplications = (): RankApplication[] => {
  try { const r = localStorage.getItem(RANK_APP_KEY); return r ? JSON.parse(r) : []; } catch { return []; }
};

const saveRankApplications = (apps: RankApplication[]): void => {
  localStorage.setItem(RANK_APP_KEY, JSON.stringify(apps));
  syncWithServer();
};

export const submitRankApplication = (userId: string, source: string, rank: string): void => {
  const apps = getRankApplications();
  // Cancel any existing pending application from this user
  const filtered = apps.filter(a => !(a.userId === userId && a.status === 'PENDING'));
  filtered.unshift({ id: randomId(), userId, source, rank, requestedAt: new Date().toISOString(), status: 'PENDING' });
  saveRankApplications(filtered);
};

export const approveRankApplication = (appId: string): void => {
  const apps = getRankApplications();
  ...
  // Apply to user
  const all  = getRawUsers();
  const user = all.find(u => u.id === app.userId);
  if (user) {
    user.officialRank = { source: app.source, rank: app.rank, approvedAt: new Date().toISOString(), approvedBy: 'admin' };
    saveUsers(all);
  }
};

export const rejectRankApplication = (appId: string, reason?: string): void => {
  const apps = getRankApplications();
  const app  = apps.find(a => a.id === appId);
  if (!app) return;
...
export const clearOfficialRank = (userId: string): void => {
  const all  = getRawUsers();
  const user = all.find(u => u.id === userId);
  if (user) { user.officialRank = undefined; saveUsers(all); }
};
...
export const exportData = (): string =>
  JSON.stringify({
    users: getRawUsers(), matches: getMatches(), settings: getSettings(),
    logs: getLogs(), rankApplications: getRankApplications(),
    timestamp: new Date().toISOString(),
  });
...
export const importData = (json: string): boolean => {
  try {
    ...
    if (d.rankApplications) localStorage.setItem(RANK_APP_KEY, JSON.stringify(d.rankApplications));
    syncWithServer(); return true;
  } catch { return false; }
};
...
export const seedData = async (): Promise<void> => {
  ...
  saveUsers(initial.map((m, i) => ({ ...newUserBase(m.name, m.reading, i < 4), id: `u${100 + i}`, faction: (i % 2 === 0 ? 'RED' : 'WHITE') as 'RED' | 'WHITE' })));
};
...
export const getUserIconDef = (id: string): IconDef => ICONS_DATA.find(i => i.id === id) || ICONS_DATA[0];
export const playSound  = (_: any): void => {};
export const vibrate    = (_: any): void => {};
export const balanceFactions = (u: any) => u;
export const toggleGeneral   = (_: any): void => {};
